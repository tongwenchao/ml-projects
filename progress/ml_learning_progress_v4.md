...
### 当前进度
- Week 8 已完成：NCF 模型训练，最佳 HR@10: 0.549，权重保存在 `../data/ncf_best_v2.pth`
- 下一步：Week 9，加入内容特征（genres / sentence-transformers）
...
│   │   └── users.dat     # 6040 个用户
│   ├── mf_best.pth       # MF 最佳模型权重（epoch 8，val RMSE 0.8934）
│   └── ncf_best_v2.pth   # NCF 最佳模型权重（epoch 9，HR@10 0.549）
└── notebooks/
...
- 数据：隐式反馈，负采样 neg_ratio=4，全量训练集 900k 条 → Dataset 450万条
- 评估：HR@10 / NDCG@10，100 候选（1正+99负），随机基准 HR=0.10
- 训练结果：最佳 HR@10 0.549，NDCG@10 0.325（epoch 9，全量数据+Dropout+weight_decay）
- 踩坑：
  - 训练集只用 100k（subset）→ epoch 2 就 overfit，HR 从 0.497 一路跌到 0.424
  - 修复：用全量数据 + Dropout(0.2) + weight_decay=1e-5，曲线变健康
  - 评估时负采样有随机性，指标有 ±0.5% 抖动属正常，不是 overfitting
  - MF 和 MLP 路径的 embedding 必须分开：MF 要求维度对齐（点积），MLP 要求自由组合（拼接），共享会导致梯度冲突
  - 评估负采样未排除验证集正样本，误采概率约 0.4%，影响极小，可接受
...
|------|------|----|------|
| 随机基准 | HR@10 | 0.10 | 理论值 |
| MF | val RMSE | 0.8934 | 显式评分，不可与下面直接比较 |
| NCF v1（100k subset）| HR@10 | 0.497 | Epoch 2 最佳，之后 overfit |
| NCF v2（全量+正则）| HR@10 | 0.549 | Epoch 9 最佳，曲线健康 |
...
- 评估协议：1正+99负，100候选中排序，对每个用户独立评估后取平均
- 标准化 ≠ 归一化（常被混用，实际不同）
...
**当前进度**：项目 A 影视推荐引擎 Week 9。Week 8 已完成 NCF，最佳 HR@10: 0.549，权重保存在 `../data/ncf_best_v2.pth`。下一步是加入内容特征：用 genres 或 sentence-transformers 编码电影信息，拼接到 item embedding，处理冷启动，搭最简版双塔模型（Two-Tower）。
...
- 项目 A Week 8：NCF（MF+MLP 并联，负采样，HR@10 0.549）

**环境**：MacBook Apple Silicon，conda，mlenv 环境，PyTorch 已装

**当前文件**：`~/ml-projects/recommender/notebooks/week9.ipynb`
...
完整学习进度文档见附件 `ml_learning_progress_v4.md`。
