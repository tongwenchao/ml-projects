...
**环境**：Vast.ai RTX 3090 24GB，CUDA 12.6，PyTorch 2.11.0，transformers 5.5.4，peft 0.19.1，bitsandbytes 0.49.2，trl 1.2.0

**数据集**：`zeroshot/twitter-financial-news-sentiment`，金融推文情感三分类
...
训练集 9543 条，验证集 2388 条。数据不均衡，评估时需看 per-label accuracy。
...
| | 参数量 |
|---|---|
| Mistral-7B 总参数 | 7,117,500,416（7.1B） |
| 可训练参数（LoRA） | 6,828,032（0.096%） |
| 显存占用（NF4量化后） | 3.9 GB（原始 BF16 需 14GB） |

**训练结果**（3 epoch，batch=16，lr=2e-4，bf16=True）：
...
Best val_acc = **0.915**（Epoch 3，按 accuracy 选模型）。Epoch 3 val_loss 上升但 accuracy 仍小幅提升，`load_best_model_at_end=True` + `metric_for_best_model="accuracy"` 正确取 Epoch 3。

**Per-label accuracy（Epoch 3）**：
- label 0 Bearish：0.879（347条）
- label 1 Bullish：0.859（475条）
- label 2 Neutral：0.940（1566条）

三个 label 均高于 85%，无退化到全猜 label 2 的情况。

**保存的文件**（`~/ml-projects/lora-finetune/mistral-qlora-sentiment-adapter/`）：

| 文件 | 大小 | 说明 |
|------|------|------|
| adapter_model.safetensors | 26 MB | A/B 矩阵权重（FP32） |
| adapter_config.json | 1.0 KB | LoRA 配置 |
| README.md | 5.1 KB | PEFT 自动生成 |
...
dtype 分工：
- 磁盘存储：BF16（Mistral-7B 原始格式）
- 显存存储：NF4（量化后，占用 3.9GB）
- 矩阵计算：FP16（`bnb_4bit_compute_dtype`，反量化时临时转换）
- LoRA A/B 矩阵：FP32（`get_peft_model` 默认，保证梯度更新精度）
- 梯度 / loss：BF16（`bf16=True`，混合精度训练）
- optimizer 更新：FP32（AdamW 内部保持 FP32 防止 underflow）

为什么只选 Q+V 不选 K：Q 控制"query 哪里"，V 控制"取什么内容"，两者覆盖 attention 的独立变化方向；K 和 Q 高度相关（都参与 QK^T），加 K 收益边际递减但参数量增加 1/3。

`bf16=True` vs `bnb_4bit_compute_dtype`：前者控制 Trainer 整体训练循环（loss、梯度）的混合精度上下文；后者控制 NF4 权重反量化时的计算格式。严格一致的写法应两者都用 BF16 或都用 FP16，混用（BF16 训练 + FP16 compute）在实践中影响不大。

**关键踩坑**：
1. **`financial_phrasebank` datasets 4.x 不兼容**：该数据集只有旧式 `.py` script，无 parquet，datasets 4.x 已禁用 script 加载。换用 `zeroshot/twitter-financial-news-sentiment`。
2. **数据不均衡评估**：整体 acc 65% 基准（全猜 Neutral），必须看 per-label acc 才能判断模型是否真正学习。
3. **`metric_for_best_model` 和 val_loss 可能选不同 epoch**：本例 val_loss 最优在 epoch 2，accuracy 最优在 epoch 3，配置决定取哪个。
...
**当前进度**：Phase 2 项目 E，Week 24，尚未开始。
...
- 项目 E Week 23：Vast.ai RTX 3090 上 QLoRA fine-tune Mistral-7B（金融情感分类，val_acc 0.915，adapter 26MB）
