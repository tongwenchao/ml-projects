...
**Week 7**：协同过滤原理
- 数据探索：评分分布、矩阵稠密度、热门电影 ← 当前位置
- 手写 Matrix Factorization：user embedding + item embedding → 点积预测评分
- 理解 embedding 本质：离散 ID → 连续向量空间
...
**当前进度**：项目 A 影视推荐引擎 Week 7，刚读入 MovieLens 1M 数据，下一步是运行数据探索 cell（评分分布、矩阵稠密度、热门电影），然后开始写 Matrix Factorization 模型。
...
**当前文件**：`~/ml-projects/recommender/notebooks/week7.ipynb`
...
完整学习进度文档见附件 `ml_learning_progress_v2.md`。