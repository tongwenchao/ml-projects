...
- 项目 E：LoRA Fine-tune（Week 22–25）⬅ 进行中（Week 24 已完成）
...
**保存文件**：`~/ml-projects/lora-finetune/mistral-qlora-sentiment-adapter/`（adapter_model.safetensors 26MB + adapter_config.json）
...
**关键踩坑（Colab 环境）**：
1. **pip 版本要和训练环境一致**：不指定版本直接装，让 pip 自动解决依赖，避免手动指定导致的兼容性问题
2. **accelerate 降版本后仍有 import 错误**：根本原因是 peft 版本与 accelerate 不匹配，解决方案是用训练时的原生版本组合
3. **OOM 后必须 Runtime → Restart session**：不 restart 直接重试会因显存未释放而反复 OOM
4. **`model.config.pad_token_id` 需手动设置**：新版 transformers 不自动从 tokenizer 同步，batch > 1 时会报错
5. **logits 是 BF16，numpy 不支持**：softmax 前需 `.float()` 转换
6. **`hf_device_map` 属性在新版 transformers 已改名**：直接去掉或用其他方式检查 device

**adapter 加载机制**（重要概念）：
- `modules_to_save: ["classifier", "score"]` 的层保存**完整权重**（非低秩矩阵），加载后通过 `ModulesToSaveWrapper` 旁路 base model 的同名层
- LoRA 层通过 `LoraLayer` 包装，forward 时计算 `W₀x + BAx`（真正相加）
- base model 里的 score.weight 加载后仍在内存，但 [3, 4096] 只有 ~48KB，可忽略
...
**当前进度**：Phase 2 项目 E，Week 25，尚未开始。
...
- 项目 E Week 24：Colab 评估（confusion matrix + 人工案例 + base model zero-shot 对比），复现 val_acc 0.9146
...
- mlx-lm 0.31.2 已安装
- adapter 在本地：`~/ml-projects/lora-finetune/mistral-qlora-sentiment-adapter/`

**注意事项**：
- 暂不使用 AWS infrastructure
- 教学风格：每次只给一个 cell 的代码，跑完看结果再继续，遇到问题用数据验证而不是猜测

完整学习进度文档见附件 `ml_learning_progress_v17.md`。
