...
### 当前进度
- Week 10 已完成：Faiss 向量召回 + FastAPI 推理接口
- 下一步：Week 11，ONNX 优化
...
└── src/
    ├── recommender.py        # 核心推理类 Recommender
    └── api.py                # FastAPI 推理接口
```
...
- 踩坑：
  ...
  5. **OpenMP 冲突**：faiss 和 torch 各自带一份 libomp，启动时报 `OMP Error #15`。解决：`KMP_DUPLICATE_LIB_OK=TRUE` 环境变量。在 FastAPI 里用 `os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"` 放在最顶部。
...
- **OpenMP 冲突**：torch + faiss 并存时设 `KMP_DUPLICATE_LIB_OK=TRUE`
...
**当前进度**：项目 A 影视推荐引擎 Week 11。Week 10 已完成 Faiss 向量召回 + FastAPI 推理接口。下一步是 ONNX 优化。
...
**src 目录已有文件**：
- `recommender.py`：核心推理类 Recommender（加载模型、建 Faiss 索引、recommend 方法）
...
- 启动服务：`KMP_DUPLICATE_LIB_OK=TRUE uvicorn api:app --reload --port 8000`
...
完整学习进度文档见附件 `ml_learning_progress_v6.md`。
